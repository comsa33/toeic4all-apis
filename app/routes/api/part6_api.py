...
async def get_passage_types(
    ...
    if used_only:
        # 실제 사용 중인 지문 유형만 반환
        result = await cached_query_service.get_part6_passage_types(use_cache=not skip_cache)
    else:
        # 정적 지문 유형 목록
        result = [
            "Email/Letter",
...
async def get_difficulties(
    ...
    if used_only:
        # 실제 사용 중인 난이도만 반환
        result = await cached_query_service.get_part6_difficulties(passage_type, use_cache=not skip_cache)
    else:
        # 기본 난이도 목록
